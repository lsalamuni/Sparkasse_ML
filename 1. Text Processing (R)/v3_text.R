...
# Imbalance metrics
cat("\nImbalance Metrics:\n")
cat("- Max/Min class ratio:", round(max(class_distribution$n) / min(class_distribution$n), 2), "\n")
cat("- Gini coefficient:", round(2 * sum(rank(class_distribution$n) * class_distribution$n) / 
                                  (length(class_distribution$n) * sum(class_distribution$n)) - 
                                  (length(class_distribution$n) + 1) / length(class_distribution$n), 3), "\n")
cat("- Classes with <10% of data:", sum(class_distribution$np < 10), "out of", nrow(class_distribution), "\n")
...
cat("\n=== FULL FEATURE SET (BEFORE REDUCTION) ===\n")
cat("Total documents:", nrow(ml_dataset_full), "\n")
cat("Total features:", ncol(ml_dataset_full) - 2, "\n")  # Minus document and category columns
cat("\nFeature breakdown:\n")
...
cat("Performing SVD with", n_components, "components...\n")
...
for(i in 1:min(10, length(explained_var))) {
  cat(sprintf("Component %d: %.2f%% (Cumulative: %.2f%%)\n", 
              i, explained_var[i]*100, cumulative_var[i]*100))
}
...
cat("\nKeeping", n_components_keep, "components (explaining", 
    round(cumulative_var[n_components_keep]*100, 2), "% variance)\n")
...
cat("Created", ncol(embedding_features) - 1, "embedding features\n")
...
cat("\nFeature breakdown:\n")
cat("- Email structure features:", sum(str_detect(names(ml_dataset), "char_count|word_count|avg_|question_|exclamation_|total_punct|sentence_|capital_|all_caps|contains_|is_|has_")), "\n")
cat("- Banking domain features:", sum(str_detect(names(ml_dataset), "_keywords")), "\n")
cat("- SVD components (from TF-IDF):", sum(str_detect(names(ml_dataset), "^SVD_")), "\n")
cat("- Embedding features:", sum(str_detect(names(ml_dataset), "^Embed_")), "\n")
...
cat("\n=== FEATURE ANALYSIS FOR IMBALANCED CLASSES ===\n")
...
cat("\nTop 10 features with largest difference between minority and majority classes:\n")
feature_comparison %>%
  head(10) %>%
  print()
...
write_xlsx(ml_dataset, "ml_dataset_v3_reduced.xlsx")

# Save the full feature set (before reduction) for comparison
write_xlsx(ml_dataset_full, "ml_dataset_v3_full.xlsx")
...
saveRDS(report, "text_processing_report_v3.rds")
